...
4. Para generar solicitudes de trabajo en local mediante el webhook real, correr:
...
7. Revisar `/ingresos` para ver datos consultados desde PaymentsApp o fallback seguro.
...
Documentacion tecnica de APIs, webhooks y comandos `curl`:
...
DriverApp esta preparada para integrarse con RiderApp, PaymentsApp, FeedbackApp y Control Plane mediante APIs. Si una app externa no esta configurada o no responde, se usan fallbacks seguros no ficticios y se registra el problema en logs.
...
- Los ingresos mostrados no se calculan localmente desde trabajos finalizados; se consultan desde PaymentsApp con cache de 60 segundos.
- Las APIs disponibles estan documentadas en `docs/APIs.md`.
