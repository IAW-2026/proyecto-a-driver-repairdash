    ...
    <section className="mt-8 lg:mt-10">
      ...
      {isOffline ? (
        ...
        <div className="mt-5 rounded-[28px] border border-highlight/10 bg-highlight/[0.04] p-8 text-center">
          <p className="text-sm text-highlight/45">
        ...
        <>
          <div className="mt-5 flex flex-col gap-4 sm:hidden">
            ...
            {hasMoreMobile && (
              <button
                ...
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-magenta/25 bg-magenta/[0.08] px-5 text-sm font-black text-highlight shadow-xl shadow-black/15 transition hover:bg-magenta/[0.12]"
              >
                <ChevronsDown
                  aria-hidden
          ...
          <div
            ref={scrollRef}
            className="mt-5 hidden gap-4 sm:flex sm:snap-x sm:flex-row sm:overflow-x-auto sm:scroll-smooth sm:pb-5 sm:[-ms-overflow-style:none] sm:[scrollbar-width:none] sm:[&::-webkit-scrollbar]:hidden"
          >
            {visible.map(
              renderRequestCard,
            )}
